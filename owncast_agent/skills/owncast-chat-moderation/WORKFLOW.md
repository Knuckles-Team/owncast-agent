...
Moderate an Owncast live chat via the owncast-agent MCP server — read chat messages, toggle message visibility, enable/disable users, grant/revoke moderator status, list connected clients, and manage IP bans. Use when the agent must remove abusive messages, silence or ban a user, promote a moderator, or audit connected chat clients. Do NOT use for reading stream/viewer telemetry (owncast-broadcast-monitoring) or KG ingestion (owncast-audience-ingestion); prefer those.
